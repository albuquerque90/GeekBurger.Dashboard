...
namespace GeekBurger.Dashboard.OrderFinishedFunction
{
    class Program
    {
        const string ServiceBusConnectionString = "{ServiceBus connection string}";
        const string QueueName = "orderfinished";
        ...
        {
            SalesPerHourModel entidade = JsonConvert.DeserializeObject<SalesPerHourModel>(Encoding.UTF8.GetString(message.Body));
            ...
            do
            {
                TableOperation operation = TableOperation.Insert(new SalesPerHourModel("salesperhourdashboard", entidade.OrderId) { OrderId = entidade.OrderId, Date = entidade.Date, Hour = entidade.Hour });
